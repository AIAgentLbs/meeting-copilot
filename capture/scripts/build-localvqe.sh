...
if [ -n "${MEETINGCOPILOT_CMAKE:-}" ]; then
    CMAKE=$MEETINGCOPILOT_CMAKE
...
if [ ! -d "$SOURCE/.git" ]; then
    if [ -n "${MEETINGCOPILOT_LOCALVQE_SOURCE:-}" ]; then
        test -d "$MEETINGCOPILOT_LOCALVQE_SOURCE/.git" \
            || { echo "MEETINGCOPILOT_LOCALVQE_SOURCE is not a git checkout" >&2; exit 1; }
        ditto "$MEETINGCOPILOT_LOCALVQE_SOURCE" "$SOURCE"
    else
...
if [ -n "${MEETINGCOPILOT_LOCALVQE_MODEL:-}" ]; then
    cp "$MEETINGCOPILOT_LOCALVQE_MODEL" "$MODEL"
elif [ ! -f "$MODEL" ]; then
...
JOBS=${MEETINGCOPILOT_LOCALVQE_JOBS:-2}
