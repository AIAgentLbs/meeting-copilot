#!/usr/bin/env swift
//
// make-icon.swift — build Resources/MeetingCopilot.icns from the same feather the app
// draws at runtime.
//
//   swift scripts/make-icon.swift Resources/MeetingCopilot.icns
//
// The feather path is copied from Sources/meetingcopilot/UI/FeatherIcon.swift rather
// than imported: this runs as a standalone script, before and independently of
// the meetingcopilot target, so it cannot link against it. The two are only three SVG
...
let work = fm.temporaryDirectory.appendingPathComponent("meetingcopilot-icon-\(UUID().uuidString)")
let iconset = work.appendingPathComponent("MeetingCopilot.iconset")
