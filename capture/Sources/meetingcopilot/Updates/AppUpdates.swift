...
/// Sparkle, wired to meetingcopilot's rule that a recording outranks an update.
///
...
///
/// The updater only exists in a real `MeetingCopilot.app` on a persistent, writable
...
final class AppUpdates: NSObject, SPUUpdaterDelegate {
    ...
    init(gate: UpdateGate) {
        self.gate = gate
        super.init()
        guard ProductBrand.updatesEnabled, Runtime.supportsPersistentFeatures else { return }
    ...
    func updater(_ updater: SPUUpdater, mayPerform updateCheck: SPUUpdateCheck) throws {
        ...
        guard allowed else {
            throw NSError(
                domain: "com.aiagentlabs.meeting-copilot.updates",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "meetingcopilot is recording; the update check can wait."]
            )
        }
    }
