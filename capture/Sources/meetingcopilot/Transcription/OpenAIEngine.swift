...
/// OpenAI's transcription models meetingcopilot can use: the others return running text
...
actor OpenAITranscriptionEngine: TranscriptionEngine {
    ...
    private func send(_ audio: URL) async throws -> Data {
        ...
        let body = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-openai-\(UUID().uuidString).multipart")
    ...
    /// The response as meetingcopilot's segments: times moved onto the session clock,
    ...
    /// The slice of the API response meetingcopilot uses. Everything else it returns —
