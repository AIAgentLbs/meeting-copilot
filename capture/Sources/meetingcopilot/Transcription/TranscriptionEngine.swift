...
/// and the voice within it (`1A`, `2B`). MeetingCopilot's stable vocabulary keeps those
...
/// paid API on every restart, which is what meetingcopilot did until 2026.08.17.
protocol TranscriptionFailure: Error {
    var isPermanent: Bool { get }
}

/// A speech-to-text engine meetingcopilot can run. Engines are prepared lazily (model
/// download + load) when the transcription queue has work and released when it
/// drains, so meetingcopilot never idles holding gigabytes of model weights.
