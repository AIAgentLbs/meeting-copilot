...
enum DoctorReport {
    ...
    /// anyone looks before quitting, replacing or reinstalling meetingcopilot
    ...
    /// which `meetingcopilot sessions` reads, is written when the recording stops.
    ///
    /// Always a warning, never a failure. `allOK` decides whether `meetingcopilot
    ...
    static func checkLiveRecording(_ root: URL) -> Check {
        ...
        if !live.isEmpty {
            ...
            return Check(
                ...
                remediation: "stop it in meetingcopilot before quitting or replacing the app: the session "
                    + "is saved either way, but nothing is recorded until meetingcopilot runs again"
            )
        }
        ...
        guard underway.isEmpty else {
            ...
            return Check(
                ...
                remediation: "start meetingcopilot: the next launch writes the meta.json that crash left "
    ...
    /// Says out loud what meetingcopilot will do on its own, because the surprising
    ...
    static func checkMicrophone() -> Check {
        ...
        case .denied, .restricted:
            return Check(
                name: "microphone",
                status: .fail("denied"),
                remediation: "System Settings → Privacy & Security → Microphone → enable for meetingcopilot (or your terminal)"
            )
        @unknown default:
    ...
    static func checkSystemAudio(isBundled: Bool, heardAt: Date?, now: Date) -> Check {
        guard isBundled else {
            return Check(
                ...
                remediation: "run MeetingCopilot.app — `make app`, then put it in /Applications"
            )
        }
        if let heardAt, heardAt <= now {
            ...
            return Check(
                ...
                remediation: "last successful tone test: \(age); run `meetingcopilot setup` to test again"
            )
        }
        return Check(
            name: "system audio",
