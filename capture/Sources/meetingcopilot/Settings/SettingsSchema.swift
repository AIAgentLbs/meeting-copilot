...
/// Every setting meetingcopilot has, described once: where it lives in the config file,
...
enum SettingsSchema {
    ...
    static var sections: [Section] { [
        ...
        Section(title: localised("Transcription", "Расшифровка"), entries: [
            ...
            Entry(["transcription", "assemblyai", "api_key_path"],
                  ...
                  .text, default: "~/.config/meeting-copilot/keys/assemblyai"),
        ...
        Section(title: localised("Summaries", "Саммари"), entries: [
            ...
            Entry(["summary", "api_key_path"],
                  ...
                  .text, default: "~/.config/meeting-copilot/keys/anthropic"),
            Entry(["summary", "openai_api_key_path"],
                  ...
                  .text, default: "~/.config/meeting-copilot/keys/openai"),
        ]),
        ...
        Section(title: localised("Calendar and naming", "Календарь и имена"), entries: [
            ...
            Entry(["calendar_provider"],
                  localised("Calendar source", "Источник календаря"),
                  localised(
                      "google reads only a Google account connected to macOS; all also includes iCloud and local calendars.",
                      "google читает только Google-аккаунт, подключённый к macOS; all также включает iCloud и локальные календари."),
                  .choice(["google", "all"]), default: "all", needsRestart: true),
        ...
        Section(title: localised("Interface", "Интерфейс"), entries: [
            ...
            Entry(["interface_language"],
                  localised("Language of meetingcopilot's own windows", "Язык окон meetingcopilot"),
                  localised(
            ...
            Entry(["dock_icon"],
                  ...
                  localised(
                      "Off makes meetingcopilot a menu-bar-only accessory — which the menu bar hides when it runs out of room.",
                      "Выключено — meetingcopilot живёт только в строке меню, а строка меню прячет значок, "
                          + "когда ей не хватает места."),
                  .toggle, default: true, askedInSetup: true),
            Entry(["menu_bar_icon"],
                  localised("Show in the menu bar", "Показывать в строке меню"),
                  localised(
                      "Off with the Dock icon too leaves meetingcopilot with no icon anywhere: open MeetingCopilot "
                          + "again to bring its window back.",
                      "Выключено вместе со значком в доке — meetingcopilot нигде не видно: чтобы вернуть "
                          + "окно, откройте MeetingCopilot ещё раз."),
                  .toggle, default: true, askedInSetup: true),
            Entry(["window"],
    ...
    /// listed here still count as known — a setting meetingcopilot obeys must never be
