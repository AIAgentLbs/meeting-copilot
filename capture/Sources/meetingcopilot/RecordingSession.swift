...
struct RecordingInProgress: Sendable {
    ...
    /// readable `started` in it; every version of meetingcopilot does.
    let started: Date?
...
final class RecordingSession {
    ...
    /// reads it — `meetingcopilot sessions`, the transcription queue — can only ever
    ...
    static func recoverInterrupted(root: URL) -> [URL] {
        ...
        for dir in entries.sorted(by: { $0.lastPathComponent < $1.lastPathComponent }) {
            ...
            // A live owner means a second meetingcopilot is recording into this folder
        ...
        if !recovered.isEmpty {
            ...
            notifyUser(
                title: localised(
                    "meetingcopilot — interrupted recording recovered",
                    "meetingcopilot — прерванная запись восстановлена"),
        ...
        for name in ["mic", "system"] {
            ...
            if size != trackSize[name] {
                ...
                if trackStalled.remove(name) != nil {
                    notifyUser(
                        title: localised(
                            "meetingcopilot: \(name) track recovered",
                            "meetingcopilot: дорожка \(name) снова пишется"),
                        body: localised(
                ...
                notifyUser(
                    title: localised(
                        "meetingcopilot: \(name) track stalled", "meetingcopilot: дорожка \(name) встала"),
                    body: localised(
    ...
    private func followCallApp(now: Date) {
        ...
        notifyUser(
            title: localised(
                "meetingcopilot: nothing from the far end", "meetingcopilot: от дальней стороны ничего нет"),
