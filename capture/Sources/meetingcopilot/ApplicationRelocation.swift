...
/// LaunchServices so macOS continues attributing permissions to MeetingCopilot.
enum ApplicationRelocation {
    ...
    /// Copy through a sibling first. If an older MeetingCopilot is already installed,
    ...
    static func offerMoveIfNeeded(bundleURL: URL? = Runtime.bundleURL) -> Bool {
        ...
        alert.messageText = localised(
            "Move MeetingCopilot to Applications?",
            "Переместить MeetingCopilot в «Программы»?")
        alert.informativeText = localised(
            "MeetingCopilot is running from the disk image. Move it first so Start at Login and automatic updates keep working after the image is ejected.",
            "MeetingCopilot запущена с образа диска. Сначала переместите её, чтобы автозапуск и автообновления работали после извлечения образа.")
        ...
        do {
            ...
            guard launch(installed) else {
                throw NSError(
                    domain: "com.aiagentlabs.meeting-copilot.install",
                    code: 1,
            ...
            failure.messageText = localised(
                "MeetingCopilot couldn't be moved", "Не удалось переместить MeetingCopilot")
