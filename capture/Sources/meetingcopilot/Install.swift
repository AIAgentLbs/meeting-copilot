...
/// way to be the process macOS holds responsible for system audio. MeetingCopilot.app
...
struct Install: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Start meetingcopilot at login, or stop it starting."
    )

    @Flag(name: .long, help: "Register meetingcopilot to start at login.")
    var launchAtLogin: Bool = false

    @Flag(name: .long, help: "Stop meetingcopilot starting at login.")
    ...
    func run() throws {
        ...
        guard Runtime.supportsPersistentFeatures else {
            throw CleanExit.message(
                "this copy cannot stay at its current path — put MeetingCopilot.app in "
                    + "/Applications and run it from there.")
        }

        if uninstall {
            try LoginItem.unregister()
            print("✓ meetingcopilot will no longer start at login")
            return
        }
        let state = try LoginItem.register()
        print(state == .needsApproval
            ? "! registered — allow meetingcopilot in System Settings → General → Login Items"
            : "✓ meetingcopilot will start at login")
    }
}
