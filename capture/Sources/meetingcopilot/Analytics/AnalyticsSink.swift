...
final class AnalyticsSink: @unchecked Sendable {
    ...
    enum Endpoint {
        static let url = URL(string: "https://stats.meetingcopilot.me/api/batch")!
    ...
    /// Beside the identifier, so that forgetting everything meetingcopilot knows about
    /// this machine stays one `rm ~/.config/meeting-copilot/analytics*.json`.
    static let defaultStore = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".config/meeting-copilot/analytics-pending.json")
    ...
    private let queue = DispatchQueue(label: "com.aiagentlabs.meeting-copilot.analytics")
    ...
    private func append(
        ...
        pending.append([
            ...
            "type": "event",
            "payload": [
                "hostname": "app.meetingcopilot.me",
                "url": "/",
