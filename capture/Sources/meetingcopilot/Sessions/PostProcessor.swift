...
enum PostProcessor {
    ...
    /// `meetingcopilot process` — and any two of them can be in the same folder at the
    ...
    /// recordings window to decide whether its button does anything, `meetingcopilot
    ...
    /// What `meetingcopilot process` should do with a session, decided before anything
    /// is printed or run.
    ///
    ...
    /// and only one of them is in English: `description` is what `meetingcopilot
        ...
        var description: String {
            ...
            case .retired(let why):
                return "This session was retired without a transcript: \(why)\n"
                    + "Transcribe it from its audio anyway with `meetingcopilot process --again`."
            }
        }
    ...
    static func markForRetranscription(_ dir: URL) {
        guard !SessionClaim.isHeld(dir) else {
            appendSessionLog(
                "not clearing for re-transcription — another meetingcopilot has this session", to: dir)
            return
        }
