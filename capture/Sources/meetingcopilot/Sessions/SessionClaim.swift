...
///
/// The app and `meetingcopilot process` are two processes over one recordings folder,
        ...
        var description: String {
            guard let holder else {
                return "Something else is already working on this recording: its "
                    + "\(SessionClaim.file) can't be read, so meetingcopilot is leaving the folder "
                    + "alone. Delete that file if no other copy of meetingcopilot is running."
            }
            ...
            guard holder.pid != ProcessInfo.processInfo.processIdentifier else {
                return "This copy of meetingcopilot is already \(what) — leaving it to the run "
                    + "that has it."
            }
            return "Another meetingcopilot (pid \(holder.pid)) is already \(what). Let it finish, "
