...
final class StatusWindow {
    ...
    init() {
        ...
        panel.title = ProductBrand.fullName
        ...
        panel.setFrameAutosaveName("meetingcopilot.status")
    ...
    /// is in the background moves it only within meetingcopilot's own layer, so a
    ...
    func update(state: MenuBarController.State, elapsed: String?) {
        ...
        icon.image = BrandIcon.image(
            size: 18,
            color: BrandIcon.color(recording: recording, paused: state == .paused)
        )
        switch state {
        case .idle:
