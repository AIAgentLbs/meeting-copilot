...
/// What language meetingcopilot's own windows are in, and how that is decided.
///
...
/// two are about other people's words; this one is about meetingcopilot's. The three
...
/// somebody who keeps their Mac in one language and wants meetingcopilot in another has
/// nowhere else to say so — the per-app override in Language & Region only
/// works for a bundle carrying `.lproj` resources, and meetingcopilot's strings are in
...
enum InterfaceLanguage: String, CaseIterable, Sendable {
    ...
    /// `ru-RU`, most wanted first. A language meetingcopilot has no words for is passed
...
/// One thing meetingcopilot says, in both languages it can say it in.
///
...
/// AssemblyAI, OpenAI, Ollama, parakeet, meetingcopilot — and so do prices, paths and
