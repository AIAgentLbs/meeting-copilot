...
/// otherwise demand at the worst possible moment, and the choices meetingcopilot can't
...
/// disagreeing about what meetingcopilot offers; and there is nothing in setup a
...
final class SetupForm: NSObject, NSTextFieldDelegate {
    ...
    private let launchRow = AccessRow(
        ...
        detail: localised(
            "So a meeting is never missed because nobody opened Meeting Copilot.",
            "Чтобы встреча не пропала из-за того, что Meeting Copilot никто не открыл."),
    ...
    private let calendarRow = AccessRow(
        title: localised("Google Calendar", "Google Календарь"),
        detail: localised(
            "Uses the Google account connected to this Mac; iCloud is excluded.",
            "Использует Google-аккаунт на этом Mac; iCloud исключён."),
        action: localised("Allow", "Разрешить"),
        grantedNote: localised("allowed", "разрешено"),
        optional: true)
    ...
    override init() {
        ...
        form.addArrangedSubview(SetupLayout.section(
            localised("Where Meeting Copilot shows up", "Где видно Meeting Copilot"),
    ...
    /// Detect first, then the five languages meetingcopilot is actually used in, then
    ...
    /// The two places a person can find a running meetingcopilot, each of which can be
    ...
    /// window: it is the only instruction meetingcopilot gives for reaching itself,
    ...
    nonisolated static func noIconsNote(menuBar: Bool, dock: Bool) -> String {
        ...
        return localised(
            "Meeting Copilot keeps recording with no icon anywhere. To bring the window back, open Meeting Copilot "
                + "again — from Spotlight, or from Applications.",
            "Meeting Copilot продолжит записывать, но его нигде не будет видно. Чтобы вернуть окно, "
                + "откройте Meeting Copilot ещё раз — из Spotlight или из папки «Программы».")
    }
    ...
    /// directory is meetingcopilot's own and mode 0700, so a key pasted here can't be
    ...
    func refresh() {
        ...
        case .unavailable:
            launchRow.update(
                .notAsked,
                detail: localised(
                    "Only Meeting Copilot.app can register itself; this is a bare build.",
                    "Зарегистрировать себя может только Meeting Copilot.app, а это голая сборка."))
        }
    ...
    nonisolated static func sentence(for outstanding: [Missing]) -> String {
        ...
        case 0:
            return localised(
                "Everything Meeting Copilot needs is granted.",
                "Всё, что нужно Meeting Copilot, разрешено.")
        case 1:
