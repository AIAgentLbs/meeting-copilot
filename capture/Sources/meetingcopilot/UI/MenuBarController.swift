...
/// a glance, and carries the menu that is meetingcopilot's fullest control surface.
///
...
final class MenuBarController {
    ...
    init(visible: Bool = true) {
        ...
        let showWindow = NSMenuItem(
            title: localised("Show Meeting Copilot", "Показать Meeting Copilot"),
        ...
        let quit = NSMenuItem(
            title: localised("Quit Meeting Copilot", "Завершить Meeting Copilot"),
    ...
    /// still outstanding, and both are answered by then. `meetingcopilot setup` resets
    ...
    private static func icon(color: NSColor?) -> NSImage? {
        BrandIcon.image(size: 16, color: color)
    }
