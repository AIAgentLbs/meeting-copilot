...
/// The small window behind **About meetingcopilot**: whose program this is, where its
...
/// before anywhere else. meetingcopilot had nowhere to answer: it runs as
...
final class AboutWindow: NSObject {
    ...
    override init() {
        ...
        panel.title = localised("About Meeting Copilot", "О Meeting Copilot")
        panel.isReleasedWhenClosed = false

        let icon = NSImageView()
        icon.image = BrandIcon.image(size: 72, color: nil)

        let name = NSTextField(labelWithString: ProductBrand.name)
        name.font = .systemFont(ofSize: 22, weight: .semibold)

        let company = NSTextField(labelWithString: "by \(ProductBrand.company)")
        company.font = .systemFont(ofSize: 12, weight: .medium)
        company.textColor = .secondaryLabelColor
        ...
        linkButtons = [link(ProductBrand.company, ProductBrand.website)]
        let links = NSStackView(views: linkButtons)
        links.orientation = .vertical
        links.alignment = .centerX
        links.spacing = 4

        let stack = NSStackView(views: [icon, name, company, version, tagline, links])
        ...
        stack.setCustomSpacing(6, after: company)
    ...
    /// The links, as they read and where they go. The window is
