...
/// Asking macOS for what meetingcopilot needs, from inside the app rather than from a
/// terminal.
///
...
/// that *Terminal* may record — and meetingcopilot, started by launchd, then records a
...
enum SetupPermissions {
    ...
    enum SystemAudioResult: Equatable {
        ...
        /// this is what a terminal-launched meetingcopilot records for a whole meeting,
    ...
    static func testSystemAudio() async -> SystemAudioResult {
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-tap-test-\(UUID().uuidString).caf")
