...
struct Setup: ParsableCommand {
    ...
    func run() throws {
        ...
        // what `meetingcopilot setup` did on the one machine that needs this branch:
        // a fresh install nobody has launched yet.
        try Run.parse([]).run()
    }
}

/// A small cross-process doorbell for `meetingcopilot setup`. Distributed notifications
/// are local to this Mac and let the CLI reuse the existing daemon instead of
/// starting a second recorder beside it.
enum SetupRequest {
    private static let show = Notification.Name("com.aiagentlabs.meeting-copilot.show-setup")
    private static let acknowledged = Notification.Name("com.aiagentlabs.meeting-copilot.show-setup-ack")
