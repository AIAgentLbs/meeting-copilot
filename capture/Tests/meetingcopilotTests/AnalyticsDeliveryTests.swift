...
struct AnalyticsDeliveryTests {
    ...
    private func event(_ index: Int) -> [String: Any] {
        ["queue_id": "event-\(index)", "type": "event", "payload": [
            "website": AnalyticsSink.Endpoint.websiteID, "hostname": "app.meetingcopilot.me",
    ...
    @Test("The real Umami endpoint acknowledges a fractional legacy event after wire normalization",
          .enabled(if: ProcessInfo.processInfo.environment["MEETINGCOPILOT_ANALYTICS_TEST_WEBSITE"] != nil))
    func liveUmamiContract() throws {
        let website = try #require(ProcessInfo.processInfo.environment["MEETINGCOPILOT_ANALYTICS_TEST_WEBSITE"])
