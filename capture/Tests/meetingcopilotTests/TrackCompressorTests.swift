...
struct TrackCompressorTests {
    ...
    private func makeTwoTrackSession() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-stereo-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeSession(seconds: Double = 2, channels: AVAudioChannelCount = 1) throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-compress-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
