...
struct InterfaceLanguageTests {
    ...
    func choosing() {
        ...
        // A language meetingcopilot has no words for is passed over rather than being
    ...
    /// The menu in the top-right corner, which is the surface meetingcopilot is
    ...
    /// meeting is not meetingcopilot's to translate, and the label in front of each
    ...
    ///
    /// The speech itself is `meetingcopilot` throughout: what was said in a meeting is
    /// not meetingcopilot's to translate, so a sentence that differed between the two
    ...
    private static func readableStatusWindow() -> [String] {
        ...
        transcript.applyPartial(
            speaker: .you, text: "meetingcopilot", startMilliseconds: 0, epoch: transcript.epoch)
        transcript.setEnabled(false)
        transcript.setEnabled(true)
        transcript.applyPartial(
            speaker: .them, text: "meetingcopilot", startMilliseconds: 10, epoch: transcript.epoch)
        ...
        // language CoreML wrote it in; only the words around it are meetingcopilot's.
    ...
    /// auto-record last did what it did — are given meetingcopilot's own name, because
    ...
    private static func readableMenuBar() -> [String] {
        ...
        menuBar.updateAutoRecord(enabled: true, decision: "meetingcopilot")

        var found: [String] = []
        for state in [MenuBarController.State.idle, .recording, .paused] {
            menuBar.update(state: state, elapsed: "1:23")
            for status: TranscriptionCoordinator.Status in [
                .idle, .transcribing(session: "meetingcopilot", queued: 0),
                .transcribing(session: "meetingcopilot", queued: 2), .failed(session: "meetingcopilot"),
            ] {
    ...
    private static func recordingsOnADiskOfTheirOwn() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-language-walk-\(UUID().uuidString)")
        ...
        func session(_ name: String, transcript: Bool) throws -> URL {
            ...
            try JSONEncoder().encode(Transcript(
                engine: "parakeet", model: "v3", created_at: "2026-08-18T09:31:00Z",
                segments: [
                    .init(speaker: "me", start_ms: 0, end_ms: 2000, text: "meetingcopilot"),
                    .init(speaker: "them A", start_ms: 2000, end_ms: 4000, text: "meetingcopilot"),
                ]
            )).write(to: dir.appendingPathComponent("transcript.json"))
            try SpeakerNames(speakers: ["them A": .init(name: "meetingcopilot", source: .manual)])
                .write(to: dir)
            try Data("# meetingcopilot\n".utf8).write(to: dir.appendingPathComponent("summary.md"))
            return dir
        }
    ...
    /// one place meetingcopilot writes running text into a window rather than a label.
    @MainActor
