...
struct RecordingRecoveryTests {
    ...
    private func makeInterruptedSession(
        ...
        let root = existingRoot ?? fm.temporaryDirectory
            .appendingPathComponent("meetingcopilot-\(UUID().uuidString)", isDirectory: true)
    ...
    func interruptedSessionIsRecovered() throws {
        ...
        // is exactly the state a crashed meetingcopilot leaves behind.
    ...
    func liveSessionIsNotTouched() throws {
        // Our own PID stands in for a second meetingcopilot mid-recording.
    ...
    func liveRecordingIsReported() throws {
        // Our own PID stands in for the meetingcopilot that is recording; nothing else
