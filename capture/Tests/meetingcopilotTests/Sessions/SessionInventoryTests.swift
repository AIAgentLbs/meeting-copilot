...
struct SessionInventoryTests {
    ...
    private static func session(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-inventory-\(UUID().uuidString)")
    ...
    func manualNamesOutrankTheModel() throws {
        ...
        fresh.speakers["me"] = .init(name: "Test User", source: .account)
        let merged = byHand.merged(with: fresh)

        #expect(merged.speakers["them A"]?.name == "Фёдор")
        #expect(merged.speakers["me"]?.name == "Test User")
    }
