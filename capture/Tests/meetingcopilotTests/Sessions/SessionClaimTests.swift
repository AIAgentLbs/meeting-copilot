...
/// with this test runner's own pid stands in for a second meetingcopilot — the trick
...
struct SessionClaimTests {
    private static func session() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-claim-\(UUID().uuidString)", isDirectory: true)
    ...
    func aLiveClaimIsNotGrantedTwice() throws {
        ...
        // Our own pid stands in for a second meetingcopilot mid-transcription.
        ...
        do {
            ...
            #expect(busy.description.contains("This copy of meetingcopilot"))
        }
        #expect(
    ...
    /// And the sentence the refusal carries, which is the whole of what `meetingcopilot
    ...
    /// a message about "another meetingcopilot" that turns out to mean this one is the
