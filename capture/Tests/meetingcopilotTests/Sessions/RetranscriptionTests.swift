...
struct RetranscriptionTests {
    ...
    private static func rawSession(seconds: Double = 2) throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-again-\(UUID().uuidString)", isDirectory: true)
    ...
    /// The bug this whole route exists to fix: `meetingcopilot process` used to read a
    ...
    /// Deleted with the Finder rather than by meetingcopilot, which reads the same to
    ...
    /// A transcribed session is left alone: `meetingcopilot process` on it is still the
    ...
    /// What the menu bar counts as its backlog. A session another meetingcopilot has is
    ...
    func pendingLeavesOutSessionsSomebodyElseHas() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-claims-\(UUID().uuidString)", isDirectory: true)
        ...
        // Our own pid stands in for a second meetingcopilot mid-transcription; 999999
