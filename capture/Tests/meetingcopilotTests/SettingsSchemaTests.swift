...
struct SettingsSchemaTests {
    ...
    func setupCoversWhatAdvancedOmits() {
        ...
        #expect(held == [
            ...
            "summary.enabled",
            // The two switches under "Where meetingcopilot shows up".
            "menu_bar_icon",
            "dock_icon",
    ...
    /// in one direction above all: a key meetingcopilot *does* read must never appear
    ...
    @Test("No setting meetingcopilot reads is reported as one it ignores")
...
struct SettingsDocumentationTests {
    ...
    @Test("The key-file settings the README advertises are settings meetingcopilot reads")
