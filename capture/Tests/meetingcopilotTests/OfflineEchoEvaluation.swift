...
///
/// MEETINGCOPILOT_ECHO_EVAL_INPUT=/absolute/stereo.m4a \
/// MEETINGCOPILOT_ECHO_EVAL_OUTPUT=/absolute/repository/.build/evaluation \
/// swift test --skip-build --filter OfflineEchoEvaluation
@Suite(
    .serialized,
    .enabled(if:
        ProcessInfo.processInfo.environment["MEETINGCOPILOT_ECHO_EVAL_INPUT"] != nil
        && ProcessInfo.processInfo.environment["MEETINGCOPILOT_ECHO_EVAL_OUTPUT"] != nil)
)
struct OfflineEchoEvaluation {
    ...
    func processStereoArchive() throws {
        ...
        let input = URL(fileURLWithPath: try #require(environment["MEETINGCOPILOT_ECHO_EVAL_INPUT"]))
            .standardizedFileURL
        let requestedOutput = URL(
            fileURLWithPath: try #require(environment["MEETINGCOPILOT_ECHO_EVAL_OUTPUT"]),
            isDirectory: true).standardizedFileURL
        let repository = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // meetingcopilotTests
        ...
        guard requestedOutput.path.hasPrefix(build) else {
            throw CocoaError(.fileWriteNoPermission, userInfo: [
                NSLocalizedDescriptionKey: "MEETINGCOPILOT_ECHO_EVAL_OUTPUT must be inside this checkout's .build directory"
            ])
        }
