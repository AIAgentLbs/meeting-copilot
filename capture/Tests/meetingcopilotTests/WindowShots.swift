...
/// so it is switched off unless `MEETINGCOPILOT_SHOTS` names a directory to write to:
///
/// ```sh
/// MEETINGCOPILOT_SHOTS=/tmp/shots swift test --filter WindowShots
/// ```
///
/// `MEETINGCOPILOT_SHOTS_LANGUAGE=ru` takes the same pictures of the Russian windows.
...
@Suite(.serialized, .enabled(if: ProcessInfo.processInfo.environment["MEETINGCOPILOT_SHOTS"] != nil))
struct WindowShots {
    private var directory: String {
        ProcessInfo.processInfo.environment["MEETINGCOPILOT_SHOTS"] ?? NSTemporaryDirectory()
    }
    ...
    private var language: InterfaceLanguage {
        ProcessInfo.processInfo.environment["MEETINGCOPILOT_SHOTS_LANGUAGE"]
    ...
    private func settingsPanel(
        ...
        owners.append(try #require(window))
        let title = localised("meetingcopilot settings", "Настройки meetingcopilot")
        let panel = try #require(NSApp.windows.last { $0.title == title })
        panel.appearance = appearance
        return panel
    }
...
/// stopped there. But meetingcopilot has five windows, and a set of pictures meant to
...
///
/// ```sh
/// MEETINGCOPILOT_SHOTS=/tmp/shots swift test --filter WindowGallery
/// ```
@Suite(.serialized, .enabled(if: ProcessInfo.processInfo.environment["MEETINGCOPILOT_SHOTS"] != nil))
struct WindowGallery {
    ...
    func statusWindow() throws {
        ...
        for (name, appearance) in [("light", light), ("dark", dark)] {
            ...
            let panel = try #require(NSApp.windows.last { $0.title == "meetingcopilot" })
            panel.appearance = appearance
    ...
    /// These pictures introduce meetingcopilot to readers of the matching-language
    ...
    private static func fixture() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("meetingcopilot-gallery-\(UUID().uuidString)")
    ...
    /// this suite's whole errand is showing somebody what meetingcopilot looks like.
    ///
